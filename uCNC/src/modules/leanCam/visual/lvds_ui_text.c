 ...
 * Purpose: text measurement, wrapping, and drawing helpers for LVDS UI rendering.
 * Called by: visual modules only.
 * Calls into: LVDS primitive text drawing.
 * Owns: no persistent state; separate from leancam_code, which parses NC program rows.
 */
