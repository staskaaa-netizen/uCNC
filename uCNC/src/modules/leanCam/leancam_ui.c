...
static bool lc_line_is_plain_command_storage(const char *line)
{
    if (!line)
        return false;
...
static void lc_strip_plain_command_field_braces(char *line)
{
    ...
    if (!line || !lc_line_is_plain_command_storage(line))
        return;
...
bool leancam_ui_commit_draft(leancam_ui_t *ui)
{
    ...
    lc_strip_plain_command_field_braces(saved);
...
void leancam_get_module_name(const char *line, char *out, int out_sz)
{
    ...
    while (line[i] && line[i] != ' ' && line[i] != '\t' && i < out_sz - 1) {
