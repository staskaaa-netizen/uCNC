 ...
 * Purpose: build the latest renderer snapshot from machine runtime and LeanCam bridge state.
 * Called by: execution_controller snapshot step.
 * Calls into: motion/runtime getters, encoder debug, and leancam_bridge_fill_snapshot.
 * Owns: latest snapshot frame only; it must not advance editor policy or draw pixels.
 */
